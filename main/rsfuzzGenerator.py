...
class InputGenerator:
    def __init__(self, grammar, depths_dict, config, recurrent_sequences, log=False):
    ...
    def expand_node_randomly(self, tree, tmp_sub_seqs):
        ...
        for i, node in enumerate(tree[-1]):
            ...
            if children is None:
                ...
                # print(deriv_num_list)
                cand = [j for j, dn in enumerate(deriv_num_list) if dn not in deriv_num_to_prune]
                if not cand:
                    cand = list(range(len(deriv_num_list)))

                use_weight = any(children_probability[j] > 0 for j in cand)
                if use_weight:
                    chosen_j = random.choices(cand, weights=[children_probability[j] for j in cand], k=1)[0]
                else:
                    chosen_j = random.choice(cand)

                new_nodes = possible_children[chosen_j]
                deriv_num = deriv_num_list[chosen_j]
    ...
    def generate_input(self, num, output_dir, tree_dir):
        n_inputs = 0
        for i in range(num):
            # node : [token, derivation number, list of children indexes, sequence_idx]
...
def main(rule_dict, depths_dict, config, input_num, output_dir, tree_dir, recurrent_sequences):
    for key, value in rule_dict.items():
        rule_dict[key] = normalize_prob(value)
    generator = InputGenerator(rule_dict, depths_dict, config, recurrent_sequences, log=False)
    # for token in generator.non_sequence_terminals:
    #     print(token, generator.non_sequence_terminals[token])
    n_inputs = generator.generate_input(input_num, output_dir, tree_dir)
    return n_inputs
...
def parse_args(
    ...
    return main(rule_dict, depths_dict, config, input_num, output_dir, tree_dir, recurrent_sequences)
