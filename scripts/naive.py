...
def naive_capture(seqs_list):
    total_cnt = len(seqs_list)
    node_cnt = {}
    for depth in seqs_list[0][0]:
        for (cur_symbol, cur_children, cur_dn, cur_pi) in depth:
            now_depth = cur_pi[0] + 1
            if now_depth == 0:
                node_info = (cur_symbol, cur_pi[0]+1, cur_dn, None)
            else:
                node_info = (cur_symbol, cur_pi[0]+1, cur_dn, seqs_list[0][0][cur_pi[0]][cur_pi[1]][0])
            node_cnt[node_info] = 1
    for tree, cnt in seqs_list:
        for depth in tree:
            for (cur_symbol, cur_children, cur_dn, cur_pi) in depth:
                now_depth = cur_pi[0] + 1
                if now_depth == 0:
                    ...
                    node_info = (cur_symbol, cur_pi[0]+1, cur_dn, tree[cur_pi[0]][cur_pi[1]][0])
                if node_info in node_cnt:
                    node_cnt[node_info] += 1
    
    removed_node_cnt = set()
    for node_info in node_cnt:
...
if __name__ == "__main__":
    ...
    naive_version = options.naive_version
